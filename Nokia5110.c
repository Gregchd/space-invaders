...
void Nokia5110_Runner(unsigned char *frames[], int Wframe, int Hframe, int Xpos, int Ypos){
    ...
    int x = 0; //centro de la pantalla
    char str[32];
    sprintf(str, "X:%0.2d\r\n", x); // convert result to string
    
    while(1){
        for(j=0;j<=N;j++){   // Lazo de control FOR para generar un retardo. 
            if( TivaES_LeePulsador(SW2) == PRESIONADO && x>10){
                x = x-1;
            }
            else if(TivaES_LeePulsador(SW1) == PRESIONADO && x<80){
                x = x+1;
            }
        ...
        // i++;
        if( ++i == 9 ) i = 0; //total de imagenes
    }
}

void Nave_mov(unsigned char *frames[], int Wframe, int Hframe, int Xpos, int Ypos){
    int i,len =  7;
    uint32_t  j, k=0, N = 1500;
    int x = 42;
    //mov de naves
    int xn = 0;
    ...
    while(1){
        ...
        for(j=0; j<=N;j++){
            ...
            if(TivaES_LeePulsador(SW2) == PRESIONADO && x>0){
                while( TivaES_LeePulsador(SW2) == PRESIONADO && x>0){
                    for(i=0; i<100000; i++){} // 0.125s
                    x = x-1;
                    drawBitmap(x, Ypos, frames[0], Wframe, len);
                    drawBitmap(0, 0, frames[1], 17, 17);
                    //enemigo
                    if(k<2){
                        xn = xn +1;
                        k=0;
                    }
                    drawBitmap(xn, 8, frames[2], 7, 7);
                    copyToScreen();
                    clearBuffer();
                }
            }
            else if(TivaES_LeePulsador(SW1) == PRESIONADO && x<77){
                while(TivaES_LeePulsador(SW1) == PRESIONADO && x<77){
                    for(i=0; i<100000; i++){} // 0.125s
                    x = x+1;
                    drawBitmap(x, Ypos, frames[0], Wframe, len);
                    drawBitmap(0, 0, frames[1], 17, 17);
        ...
        //drawBitmap(x, Ypos, frames[0], Wframe, len);
        //copyToScreen();
        //for(i=0; i<800000; i++){} // 1s
        //k++;
    }
}

void Enemigo_mov(unsigned char*frames[]){
    int xp = 15;
    while(1){
        uint32_t i;
        drawBitmap(xp, 8, frames[2],7,7);
        for(i=0; i<1600000; i++){} // 1s
        xp = xp + 1;
        copyToScreen();
        clearBuffer();
    }
...
void EyN_mov(unsigned char*frames[], int Wframe, int Hframe, int Xpos, int Ypos, int v){

    int i=0,j=0, len = 7;
    int x =42;
    int e1 = 0, e2 = 10, e3 = 20, e4 = 30, e5 = 40;
    int ey = 8;

    while(1){
        //Nave
        if(TivaES_LeePulsador(SW2) == PRESIONADO && x>0 && i>15){
            //for(i=0; i<100000; i++){}
            x= x-1;
            i = 0;
        }
        else if(TivaES_LeePulsador(SW1) == PRESIONADO && x<77 && i>15){
            x= x+1;
            i=0;
        }
        //Enemigos
        if(j>v){ //v = velocidad (120,60,30)
            if(ey == 8){
                e1 ++;
                e2 ++;
                e3 ++;
        ...
        //enemigos
        //primer tramo
        if((e1+7 == 83) || (e2+7 == 83) || (e3+7 == 83) || (e4+7 == 83) || (e5+7 == 83) && (ey == 8)){
            ey = 16;
        }
        //segundo tramo
        else if((e1 == 1) || (e2 == 1) || (e3 == 1) || (e4 == 1) || (e5 == 1) && (ey == 16)){
            if(ey == 16){
                ey = 24;
            }
        }
        //tercer tramo
        else if((e1+7 == 83) || (e2+7 == 83) || (e3+7 == 83) || (e4+7 == 83) || (e5+7 == 83) && (ey == 24)){
            if(ey == 24){
                break;
                //ey = 32;
            }
        ...
        //cuarto tramo
        //else if((e1 == 1) || (e2 == 1) || (e3 == 1) || (e4 == 1) || (e5 == 1) && (ey == 32)){
            //if(ey == 32){
                //break;
            //}
        //}

        drawBitmap(e1, ey, frames[2],7,7);
        drawBitmap(e2, ey, frames[2],7,7);
        drawBitmap(e3, ey, frames[2],7,7);
        drawBitmap(e4, ey, frames[2],7,7);
        drawBitmap(e5, ey, frames[2],7,7);
